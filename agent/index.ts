...
import { object2string } from "./common.js";
import { hook_okhttp } from "./hook_okhttp.js";
...
function disablePinning(){
    ...
    const packagePcMap:{[key:string]:number} = {
        ...
        "fast.cngubne9.a771u9g3rw":  0x5dc570,
...
function hook_huanxin(){
    Java.perform(function() {
        ...
        EMChatRoomManager.joinChatRoom.overload('java.lang.String', 'com.hyphenate.EMValueCallBack').implementation = function(chatRoomId:string, bool:boolean, reason:string, listener:any) {
...
function object2string(obj: any, maxDepth: number = 3, currentDepth: number = 0): string {
    ...
    try {
        ...
        var EMAChatConfig = Java.use("com.hyphenate.chat.adapter.EMAChatConfig");

        EMAChatConfig.getAppKey.implementation = function() {

            // hook getAppKey
            var appKey =  this.getAppKey.call(this);
            console.log("getAppKey:" + appKey);
            return appKey;
        };
...
function hookRongIM(){
    Java.perform(function() {
        ...
        // RCIMIWEngine.init.overload('android.content.Context', 'java.lang.String', 'boolean', 'java.lang.Boolean').implementation = function(context:any, rongkey:string, z10:boolean, bool:any) {
        //     console.log("init - rongkey: " + rongkey);
        //     // hook joinExistChatRoom
        //     return this.init.call(this, context, rongkey, z10,bool);
        // };

        RCIMIWEngine.connect.overload('java.lang.String', 'int', 'cn.rongcloud.im.wrapper.callback.RCIMIWConnectCallback').implementation = function(token:string, timeLimit:number, callback:any) {
            console.log("connect - token: " + token);
            // hook joinExistChatRoom
            return this.connect.call(this, token, timeLimit, callback);
        };

        RCIMIWEngine.joinChatRoom.overload('java.lang.String', 'int', 'boolean', 'cn.rongcloud.im.wrapper.callback.IRCIMIWJoinChatRoomCallback').implementation = function(chatRoomId:string, defMessageCount:number, z:any, listener:any) {
            console.log("joinChatRoom - chatRoomId: " + chatRoomId);
            // hook joinExistChatRoom
            return this.joinChatRoom.call(this, chatRoomId, defMessageCount, z, listener);
        };
...
function hookNim(){
    Java.perform(function() {
        ...
        NIMClient.init.implementation = function(context:any, loginInfo:any, options:any){
            if(loginInfo){
                console.log("init - loginInfo: " + object2string(loginInfo));
            }
            console.log("init - appKey: " + object2string(options));
        ...
        var AuthService = Java.use("com.netease.nimlib.biz.f.a");
            ...
            if (memmem) {
                Interceptor.attach(memmem, {
                    ...
                    onLeave: function(retval) {
                        ...
                        if (this.needle) {
                            // 使用Frida的NativePointer方法处理字节数据
                            var needlePtr = Memory.alloc(this.needle.length);
                            needlePtr.writeByteArray(this.needle);
                            var needleStr = needlePtr.readUtf8String(Math.min(this.needle.length, 32)) || '';
                            if (fridaFeatures.some(f => needleStr.toLowerCase().includes(f.toLowerCase()))) {
